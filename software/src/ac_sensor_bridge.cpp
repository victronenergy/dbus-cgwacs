...
static bool roleFromDBus(DBusBridge*, QVariant &v)
{
	...
	return (QStringList() << "grid" << "pvinverter" << "genset" << "acload" << "evcharger" << "heatpump").contains(s);
}
	...
	DBusBridge(getServiceName(acSensor, settings, isSecondary), true, parent)
{
	...
	produce("/AllowedRoles", isSecondary ? (QStringList() << "pvinverter" << "acload" << "evcharger" << "heatpump") : (QStringList() << "grid" << "pvinverter" << "genset" << "acload" << "evcharger" << "heatpump"));
...
bool AcSensorBridge::toDBus(const QString &path, QVariant &value)
{
	...
	if (value.typeId() == QVariant::Double && !qIsFinite(value.toDouble()))
		value = QVariant();
	return true;
}
...
void AcSensorBridge::producePowerInfo(AcSensorPhase *pi, const QString &path, bool isGridmeter)
{
	...
	produce(pi, "powerFactor", path + "/PowerFactor", "", 3);
